...
config_presets = {
   'edm2-img64-xs':         dnnlib.EasyDict(channels=128, lr=0.0010, betas=(0.9, 0.99), decay=2000, dropout=0.40, dropres=16, mean=-0.8, std=1.6),
...
@click.option('--ect2',          help='Activate ect2 [default: False]',                             type=bool, default=False, show_default=True)
...
@click.option('-double_mu',      help='How fast DM to CM', metavar='TICKS',                         type=click.IntRange(min=1), default=100, show_default=True)
...
@click.option('-rbatch',         help='Size of reference batch', metavar='INT',                     type=int,  default=128,   show_default=True)  
@click.option('-stf',            help='Activate stable target field', metavar='FLOAT',              type=bool, default=False, show_default=True)   
@click.option('-norm',           help='Freeze normalizatoin layers, [default: False]',              type=bool, default=False, show_default=True)
@click.option('-conv',           help='Freeze conv layers, [default: False]',                       type=bool, default=False, show_default=True)
@click.option('-qkv',            help='Freeze qkv projection layers, [default: False]',             type=bool, default=False, show_default=True)
@click.option('-skip',           help='Freeze skip connection layers, [default: False]',            type=bool, default=False, show_default=True)
...
def main(**kwargs):
    ...
    if opts.ect2:
        c.loss_kwargs = dnnlib.EasyDict(P_mean=opts.mean, P_std=opts.std, q=opts.q, c=opts.c, k=opts.k, b=opts.b, adj=opts.mapping, wt=opts.wt, gamma=opts.gamma, initial=opts.initial, mu_type=opts.mu_schedule)
    else:
    ...
    if opts.scheme == 'ect':
        if opts.ect2:
            c.loss_kwargs.class_name = 'training.loss.ECT2Loss'
        else:
    ...
    if opts.stf:
        c.update(stf=True, rbatch=opts.rbatch)
